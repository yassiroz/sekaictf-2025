...
socketio = SocketIO(app, cors_allowed_origins="*", async_mode='threading', logger=not Config.DEBUG, engineio_logger=not Config.DEBUG)
...
def upload():
    try:
        cmd = device.execute_script('echo 1', timeout=3)
        if cmd.stdout.decode().strip() != '1':
...
class QueueThread(Thread):
    ...
    def _do_work(self, q: Queue):
        ...
        chall_app = device.application(q.client.PACKAGE_NAME)
        if not chall_app.is_installed():
            ...
            device.upload_file(apk_path, '/data/local/tmp/challenge.apk')
            device.install_local_file('/data/local/tmp/challenge.apk')
            device.delete_file('/data/local/tmp/challenge.apk')
        ...
        device.upload_file(apk_path, '/data/local/tmp/poc.apk')
        device.install_local_file('/data/local/tmp/poc.apk')
        device.delete_file('/data/local/tmp/poc.apk')

        poc_app = device.application(package_name)
